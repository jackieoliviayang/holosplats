...
class SplatfactoModelConfig(ModelConfig):
    ...
    """whether to limit gaussians to isotropic ones only"""
    fixed_size: bool = False
    """whether to fix the size of the Gaussians to the average of the optimized isotropic ones"""
...
class SplatfactoModel(Model):
    ...
    def populate_modules(self):
        ...
        if self.config.isotropic == False:
           scales = torch.nn.Parameter(torch.log(avg_dist.repeat(1, 3)))
        else:
           if self.config.fixed_size == False:
                scales = torch.nn.Parameter(torch.log(avg_dist))
           else:
                scales = torch.full_like(avg_dist, 0.0015457950288201318)
    ...
    @property
    def scales(self):
        if self.config.isotropic == True and self.config.fixed_size == False: 
            standard_scales = self.gauss_params["scales"]
            isotropic_scales = standard_scales.repeat(1, 3)
            return isotropic_scales
        if self.config.fixed_size == True:
            return self.fixed_scales.mean()*torch.ones_like(self.gauss_params["scales"].repeat(1, 3))
    ...
    def get_gaussian_param_groups(self) -> Dict[str, List[Parameter]]:
        ...
        names_list = ["means", "scales", "quats", "features_dc", "features_rest", "opacities"]
        return { 
